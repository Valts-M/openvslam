...
void stereo_tracking(const std::shared_ptr<openvslam::config>& cfg,
    ...
    rs2::config config;
    config.enable_stream(RS2_STREAM_FISHEYE, 1, 848, 800, RS2_FORMAT_Y8, 30);
    ...
    std::thread thread([&]() {
      while(true) {
        ...
        cv::imshow("fish1", input1);
        cv::imshow("fish2", input2);
...
int main(int argc, char* argv[]) {
    ...
    // run tracking
     if (cfg->camera_->setup_type_ == openvslam::camera::setup_type_t::Stereo) {
        stereo_tracking(cfg, vocab_file_path->value(), auto_term->is_set(),
